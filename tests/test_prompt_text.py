...
# what is missing.
#
# `classify_response`'s and `grade_sharper`'s `response` rows were briefly REMOVED (T2, the
# collapsed Push/Student-reply template) on the theory that a message with no engine heading left
# at all needs no seam to guard. The founder reverted that collapse (see model.py's own comment on
# `classify_response`): both sites route `response` through `labelled` again, so both rows are
# restored below, watched exactly like every other site in this list.
