...
# hands to prompt_text.bulleted (model.py's own _bulleted wrapper before boundary-7 Fix 2; the two
# were byte-equivalent, so this comment's measurements hold unchanged), not what reaches the
# prompt: bulleted indents every continuation line past the bullet, and a newline-heavy position
# can render up to ~5x larger than its capped length (measured against a pathological 1200-char,
# all-newline position: 5999 rendered chars, not 1200). 8 such positions render to roughly 48,000
# characters of learner text, not the naive 8 x 1200 = 9600 this comment used to claim.
