...
#
# 2000 no longer matches `model._TURN_RENDER_CAP` (raised to 40000 as of boundary-6 -- see
# model.py: a cap on a Vera-authored reply fed back through `recent` on every later call needs
# headroom for the model's own output ceiling; that concern does not apply here). Every blob this
# caps -- her situation, her focus, her committed positions -- is HER typed text, never something
# the model wrote back, so there is no output-ceiling floor this number has to clear; 2000 stays
# generous against a real typed message on its own terms (a sentence or two in the ordinary case).
# The positions block gets the same cap rather than a smaller one: it is the sum of one committed
